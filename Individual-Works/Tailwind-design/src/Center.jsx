import App from "./App";

function Center() {
    return <div class="lg:flex mt-4 grid place-items-center lg:grid-cols-2">
        <img src="https://tse4.mm.bing.net/th?id=OIP.Ix6XjMbuCvoq3EQNgJoyEQHaFj&pid=Api&P=0" class="mr-6" />
        <div class="w-48">
            <p class="text-xs">malý nadpis</p>
            <p class="text-3xl font-bold">velký nadpis</p>
            <div class="flex mt-4">
                <img src="https://tse3.mm.bing.net/th?id=OIP.B79qqGp1Wi0S6Ga-XI_MvwHaHa&pid=Api&P=0" class="h-8 w-8 rounded-lg" alt="" />
                <div class="ml-4">
                    <p class="text-xl font-bold ">nadpis</p>
                    <p class="overflow-ellipsis text-xs" >text gjef jhsfejsfjsefj  sefjhgsjfgsjh gfjsgfjsgfjhsfjhsgdj hfgjsdgfjsdjfgwue íwušéíqf uhiakcnoq hufgisbkůcj ihw=i uf</p>
                </div>
            </div>
            <div class="flex mt-8">
                <img src="https://tse3.mm.bing.net/th?id=OIP.B79qqGp1Wi0S6Ga-XI_MvwHaHa&pid=Api&P=0" class="h-8 w-8 rounded-lg" alt="" />
                <div class="ml-4">
                    <p class="text-xl font-bold ">nadpis</p>
                    <p class="overflow-ellipsis text-xs" >text gjef jhsfejsfjsefj  sefjhgsjfgsjh gfjsgfjsgfjhsfjhsgdj hfgjsdgfjsdjfgwue íwušéíqf uhiakcnoq hufgisbkůcj ihw=i uf</p>
                </div>
            </div>
        </div>
    </div>
}
export default Center